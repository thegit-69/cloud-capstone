...
// --- REAL API SETUP ---
// This object makes real HTTP requests to your backend server.
const API_URL = 'http://localhost:3001/api';

const api = {
  login: async (username, password) => {
    const response = await axios.post(`${API_URL}/login`, { username, password });
    return response.data; // The backend already formats this as { success, user }
  },
  registerUser: async (username, password, role, name) => {
    const response = await axios.post(`${API_URL}/users`, { username, password, role, name });
    // We'll wrap it in a success object to match the old mock format
  ...
  updatePatient: async (patient) => {
    // Note: A PUT request to /api/patients/:id would be more RESTful for updates.
    // This is simplified for the capstone project.
    console.warn("Patient update is not yet implemented in the backend API.");
    return patient;
  },
...
const Card = ({ title, children, className, actions }) => ( <div className={`bg-white rounded-xl shadow-md ${className || ''}`}> {title && <div className="p-6 border-b border-gray-200 flex justify-between items-center"><h2 className="text-xl font-bold text-gray-800">{title}</h2><div>{actions}</div></div>} <div className="p-6">{children}</div> </div> );
const StatCard = ({ icon, label, value, color }) => ( <div className="bg-white rounded-xl shadow-md p-4 flex items-center"> <div className={`rounded-full p-3 mr-4 ${color}`}>{icon}</div> <div> <p className="text-sm text-gray-500">{label}</p> <p className="text-2xl font-bold text-gray-800">{value}</p> </div> </div> );

// --- AuthScreen, VIEWS, and AppLayout (No logical changes, but now use the real 'api' object) ---
...
const AppLayout = ({ user, onLogout }) => {
  ...
  const handleUpdatePatient = async (updatedPatient) => { const result = await api.updatePatient(updatedPatient); setPatients(patients.map(p => p.id === result.id ? result : p)); setSelectedPatient(result); };
  const handleAddRecord = async (newRecord) => { const result = await api.addMedicalRecord(newRecord); setMedicalRecords(prev => [...prev, result]); };
  ...
  const NavLink = ({ viewName, children, requiredRoles }) => { if (requiredRoles && !requiredRoles.includes(user.role)) return null; return <button onClick={() => { setActiveView(viewName); setSelectedPatient(null); }} className={`w-full text-left px-4 py-2 rounded-md text-sm font-medium transition-colors duration-200 ${activeView === viewName ? 'bg-indigo-600 text-white' : 'text-gray-600 hover:bg-indigo-100'}`}>{children}</button>; };
  ...
  const renderView = () => {
    ...
    switch (activeView) {
      ...
      case 'users': return <UserManagementView users={users} setUsers={handleSetUsers} />;
  ...
  return ( <div className="min-h-screen bg-gray-50 font-sans text-gray-900"> <div className="flex"> <aside className="w-64 bg-white shadow-md h-screen sticky top-0 flex flex-col"> <div className="p-6 text-center border-b"><h1 className="text-xl font-bold text-indigo-600">Health Care Cloud</h1></div> <div className="p-4 border-b"><p className="text-sm font-semibold text-gray-700">Welcome, {user.name}</p><p className="text-xs text-gray-500 bg-gray-100 inline-block px-2 py-1 rounded-full mt-1">{user.role}</p></div> <nav className="flex-1 p-4 space-y-2"> <NavLink viewName="dashboard" requiredRoles={['Admin', 'Doctor', 'Receptionist']}>Dashboard</NavLink> <NavLink viewName="appointments" requiredRoles={['Admin', 'Doctor', 'Receptionist']}>Appointments</NavLink> <NavLink viewName="patients" requiredRoles={['Admin', 'Doctor', 'Receptionist']}>Patient Records</NavLink> <NavLink viewName="users" requiredRoles={['Admin']}>User Management</NavLink> </nav> <div className="p-4 border-t"><button onClick={onLogout} className="w-full text-left px-4 py-2 rounded-md text-sm font-medium text-red-600 hover:bg-red-100">Logout</button></div> </aside> <main className="flex-1 p-8">{renderView()}</main> </div> </div> );
}
